...
//|                                                                  |
//| Changes vs v1.4 (SIGNAL MECHANIC CHANGE):                        |
//|  - New input Min_Confluence_Count (default 3).                   |
//|  - BUY/SELL no longer fire on any-of-4 OR. They now fire only    |
//|    when >= Min_Confluence_Count arms pass simultaneously.        |
//|  - The 4 arms are unchanged (StochRSI, BB%B, RSI extreme,        |
//|    swing-low for BUY / swing-high for SELL).                     |
//|  - RSI direction gate (RSI<50 for BUY, >50 for SELL) still acts  |
//|    as a hard gate before the confluence vote — not a vote.       |
//|  - Setting Min_Confluence_Count=1 exactly reproduces v1.4.       |
//|  - SIGNAL diag log now also prints buy_arms/sell_arms counts.    |
//|  - Log file default audcad_v1_4.csv -> audcad_v1_5.csv.          |
//|  - CSV header carries ea_version=v1.5 (column layout unchanged). |
//|                                                                  |
//| All other mechanics (gate, grid, exit, lot formula, FitCheck,    |
//| emergency exit, basket reconstruction) are bit-for-bit identical |
//| to v1.4.                                                         |
//+------------------------------------------------------------------+
...
input string          Log_File                 = "audcad_v1_5.csv"; // Log_File: CSV log filename
...
// v1.5 CSV format: same as v1.4 — 14 base columns + 2 trailing fields (base_lot, account_type_tag).
// Only the header line's ea_version changes (v1.4 -> v1.5).
...
void ReconstructBaskets()
{
    ...
    if(g_long.active && g_short.active)
        Print("WARNING: both LONG and SHORT baskets reconstructed — v1.2/v1.3/v1.4/v1.5 is single-basket only. "
...
int OnInit()
{
    ...
    {
        Alert("AUDCAD v1.5: Abort_If_Standard_Account=true and contract_size=", g_contract_size,
    ...
    if(base_now <= 0.0)
    {
        if(Auto_Compute_Lot_Size_Based_On_Equity)
            Alert("AUDCAD v1.5: equity (", DoubleToString(eq,2),
    ...
    Print("v1.5: equity-scaled base lot, cent-account default, N-of-4 signal confluence. ",
          (Auto_Compute_Lot_Size_Based_On_Equity
