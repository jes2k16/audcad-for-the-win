//+------------------------------------------------------------------+
//|                                         AUDCAD_M15_v1_6.mq5     |
//|                     AUDCAD Mean-Reversion EA — v1.6              |
//|                                                                  |
//| Signal:  strategy/AUDCAD_M15_v1.md   §1-5   (base arms)          |
//| Confl:   strategy/AUDCAD_M15_v1.5.md §1     (N-of-4 confluence)  |
...
//| ATR:     strategy/AUDCAD_M15_v1.6.md     (ATR ladder pause)     |
//| Risk:    plans/1.initial requirements.md §9,10                  |
//|                                                                  |
//| Changes vs v1.5 (GRID-ADD MECHANIC CHANGE):                      |
//|  - New ATR ladder pause: while a basket is open, grid-leg adds   |
//|    are PAUSED when short-term volatility spikes (fast candles).   |
//|  - Metric: ATR(fast) / ATR(slow) on the signal timeframe. Pause  |
//|    when ratio >= ATR_Pause_Ratio (and atr_fast >= floor).         |
//|  - Resume (HYBRID): ratio must fall back to <= ATR_Resume_Ratio   |
//|    AND stay there for ATR_Resume_Confirm_Bars consecutive bars.   |
//|  - Resume add = Option A: because CheckAdd adds at most one leg   |
//|    per bar and re-baselines last_price, skipping adds while       |
//|    paused makes the first calm bar drop ONE leg at the stabilized |
//|    price automatically (re-baseline for free).                    |
//|  - Emergency-DD exit and +10-pip TP close stay live during pause. |
//|  - New inputs Enable_ATR_Pause / ATR_Period_Fast / ATR_Period_Slow|
//|    / ATR_Pause_Ratio / ATR_Resume_Ratio / ATR_Resume_Confirm_Bars |
//|    / ATR_Pause_Min_Pips.                                          |
//|  - Setting Enable_ATR_Pause=false reproduces v1.5 exactly.        |
//|  - New log events ATR_PAUSE / ATR_RESUME / ADD_PAUSED.            |
//|  - Log file default audcad_v1_5.csv -> audcad_v1_6.csv.          |
//|  - CSV header carries ea_version=v1.6 (column layout unchanged). |
//|                                                                  |
//| All other mechanics (gate, signal confluence, exit, lot formula, |
...
// §1.5 Signal confluence — v1.5
input int             Min_Confluence_Count = 3;  // Min_Confluence_Count: minimum arms (of 4) that must agree to fire. 1 = legacy v1.4 OR behavior; 3 = strict confluence; 4 = unanimous
...
input bool            Abort_If_Standard_Account             = true;  // Abort_If_Standard_Account: true = INIT_FAILED if SYMBOL_TRADE_CONTRACT_SIZE looks like a standard symbol (>10000). Default ON: v1.6 is designed for cent.
...
input string          Log_File                 = "audcad_v1_6.csv"; // Log_File: CSV log filename
...
// v1.6: ATR pause handles + state
int  h_atr_fast = INVALID_HANDLE;
int  h_atr_slow = INVALID_HANDLE;
bool   g_atr_paused        = false;  // currently pausing grid adds
int    g_atr_calm_count    = 0;      // consecutive calm bars while paused
double g_last_atr_ratio    = 0.0;    // cached ATR_fast/ATR_slow for SIGNAL log
double g_last_atr_fast_pips = 0.0;   // cached ATR_fast in pips
...
// v1.5 diag: per-bar arm-pass counts cached for SIGNAL log
int    g_last_buy_arms  = 0;
int    g_last_sell_arms = 0;
...
// v1.6 CSV format: same as v1.4/v1.5 — 14 base columns + 2 trailing fields (base_lot, account_type_tag).
// Only the header line's ea_version changes (v1.5 -> v1.6).
...
void ReconstructBaskets()
{
    ...
    if(g_long.active && g_short.active)
        Print("WARNING: both LONG and SHORT baskets reconstructed — v1.2/v1.3/v1.4/v1.5/v1.6 is single-basket only. "
...
// Runs once per M15 bar close, before RunTree. Updates g_atr_paused using a
// self-normalizing ATR ratio with a hysteresis dead-band and a confirm-bar count.
// Pausing only affects grid-leg adds (CheckAdd) — the emergency-DD exit and the
// +10-pip TP close are never paused.
void UpdateAtrPauseState()
{
    if(!Enable_ATR_Pause)
    {
        g_atr_paused     = false;
    ...
    if(CopyBuffer(h_atr_fast, 0, 1, 1, af) < 1) return;
    if(CopyBuffer(h_atr_slow, 0, 1, 1, as) < 1) return;

    double atr_fast = af[0];
    double atr_slow = as[0];
    double ratio    = (atr_slow > 0.0) ? atr_fast / atr_slow : 0.0;

    g_last_atr_ratio     = ratio;
    g_last_atr_fast_pips = (g_pip > 0.0) ? atr_fast / g_pip : 0.0;

    // Absolute floor: a high ratio on a tiny absolute move shouldn't pause.
    bool above_floor = (ATR_Pause_Min_Pips <= 0.0) ||
                       (g_last_atr_fast_pips >= ATR_Pause_Min_Pips);
    ...
    if(!g_atr_paused)
    {
        if(ratio >= ATR_Pause_Ratio && above_floor)
        {
            g_atr_paused     = true;
            g_atr_calm_count = 0;
            WriteLog("ATR_PAUSE", "-", close, 0, eq, 0, 0, 0, 0,
                     "ratio=" + DoubleToString(ratio,2) +
                     " atr_pips=" + DoubleToString(g_last_atr_fast_pips,1));
        }
    }
    else
    {
        if(ratio <= ATR_Resume_Ratio) g_atr_calm_count++;
        else                          g_atr_calm_count = 0;

        if(g_atr_calm_count >= ATR_Resume_Confirm_Bars)
        {
            g_atr_paused     = false;
            g_atr_calm_count = 0;
            WriteLog("ATR_RESUME", "-", close, 0, eq, 0, 0, 0, 0,
                     "ratio=" + DoubleToString(ratio,2) +
                     " atr_pips=" + DoubleToString(g_last_atr_fast_pips,1));
        }
    }
...
bool EvalSignal(bool &buy_f, bool &sell_f)
{
    ...
    int buy_arms = 0;
    if(stoch_k        <= Buy_Fire_If_StochRSI_K_LessThan_EqualTo) buy_arms++;
    if(pctb           <= Buy_Fire_If_Bollinger_LessThan_EqualTo)  buy_arms++;
    if(rsi14          <= Buy_Fire_If_RSI_LessThan_EqualTo)        buy_arms++;
    if(dist_swing_low <= Swing_Low_DistPips)                      buy_arms++;

    int sell_arms = 0;
    if(stoch_k   >= Sell_Fire_If_StochRSI_K_GreaterThan_EqualTo) sell_arms++;
    if(pctb      >= Sell_Fire_If_Bollinger_GreaterThan_EqualTo)  sell_arms++;
    if(rsi14     >= Sell_Fire_If_RSI_GreaterThan_EqualTo)        sell_arms++;
    if(dist_swing <= Swing_High_DistPips)                        sell_arms++;

    g_last_buy_arms  = buy_arms;
    g_last_sell_arms = sell_arms;

    if(rsi14 < Buy_Only_If_RSI_LessThan)
        buy_f = (buy_arms >= Min_Confluence_Count);

    if(rsi14 > Sell_Only_If_RSI_GreaterThan)
        sell_f = (sell_arms >= Min_Confluence_Count);
...
void CheckAdd(BasketState &bsk)
{
    ...
    // v1.6: ATR ladder pause — skip the add (and DO NOT advance last_price) while
    // volatility is elevated. On the first calm bar this function adds exactly one
    // leg at the current price and re-baselines last_price = add_px automatically
    // (the "Option A" single-leg re-baseline behavior).
    if(Enable_ATR_Pause && g_atr_paused)
    {
        if(Verbosity >= 2)
            WriteLog("ADD_PAUSED", bsk.is_long?"LONG":"SHORT",
                     iClose(g_sym,Signal_Timeframe,1), 0,
                     AccountInfoDouble(ACCOUNT_EQUITY), 0, bsk.wavg, bsk.legs, 0,
                     "atr_ratio=" + DoubleToString(g_last_atr_ratio,2) +
                     " atr_pips=" + DoubleToString(g_last_atr_fast_pips,1));
        return;
    }
...
void RunTree(bool buy_f, bool sell_f)
{
    ...
    if(g_long.active)
    {
        if(!CheckCloseTarget(g_long))   // not at target yet
            CheckAdd(g_long);           // check grid add (may be ATR-paused)
        return;
    }
...
{
    RefreshGate();
    UpdateAtrPauseState();   // v1.6: refresh ATR pause/resume state machine
    ...
    if(Verbosity >= 2)
    {
        ...
        string arm = StringFormat("rsi=%.1f stoch=%.1f pctb=%.2f swhi=%.0f swlo=%.0f buy_arms=%d sell_arms=%d min=%d",
                                  g_last_rsi, g_last_stoch, g_last_pctb,
                                  g_last_dist_swing, g_last_dist_swing_lo,
                                  g_last_buy_arms, g_last_sell_arms, Min_Confluence_Count);
        string atr = Enable_ATR_Pause
                     ? StringFormat(" atr_ratio=%.2f atr_paused=%s",
                                    g_last_atr_ratio, g_atr_paused?"1":"0")
                     : "";
...
int OnInit()
{
    ...
    Print("[ATR_PAUSE] enabled=", Enable_ATR_Pause,
          ...
          " confirm_bars=", ATR_Resume_Confirm_Bars,
          " min_pips=", DoubleToString(ATR_Pause_Min_Pips,1));

    if(Abort_If_Standard_Account && g_contract_size > 10000.0)
    {
        Alert("AUDCAD v1.6: Abort_If_Standard_Account=true and contract_size=", g_contract_size,
    ...
    if(base_now <= 0.0)
    {
        if(Auto_Compute_Lot_Size_Based_On_Equity)
            Alert("AUDCAD v1.6: equity (", DoubleToString(eq,2),
    ...
    // v1.6: reset pause state for a clean run.
    g_atr_paused     = false;
    g_atr_calm_count = 0;
    ...
    Print("v1.6: equity-scaled base lot, cent-account default, N-of-4 signal confluence, ATR ladder pause. ",
          (Auto_Compute_Lot_Size_Based_On_Equity
...
{
    if(h_atr_fast != INVALID_HANDLE) { IndicatorRelease(h_atr_fast); h_atr_fast = INVALID_HANDLE; }
    if(h_atr_slow != INVALID_HANDLE) { IndicatorRelease(h_atr_slow); h_atr_slow = INVALID_HANDLE; }
...
void OnTick()
{
    // Emergency DD check on every tick (plans/1 §9) — NEVER paused by ATR.
    double eq = AccountInfoDouble(ACCOUNT_EQUITY);
    if(eq > 0)
    {
        if(g_long.active)
