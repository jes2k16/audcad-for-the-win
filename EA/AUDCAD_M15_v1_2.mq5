...
//| Risk:    plans/1.initial requirements.md §9,11,12               |
//|                                                                  |
//| Changes vs v1.1:                                                 |
//|  - Exit: weighted-avg +BasketTPPips target (was: opp signal)     |
//|  - No auto-close of opposite basket on signal                    |
//|  - Single basket at a time (was: bidirectional)                  |
//+------------------------------------------------------------------+
...
input string          LogFile          = "audcad_v1_2.csv";
...
// Strategy v1 §4: 1x / 24x / 36x / 48x / 60x / ...
double LegLot(int n)
{
...
void ReconstructBaskets()
{
    ...
    if(g_long.active && g_short.active)
        Print("WARNING: both LONG and SHORT baskets reconstructed — v1.2 is single-basket only. "
...
int OnInit()
{
    ...
    else
    {
        FileWriteString(g_log,
            "# account_type=standard,symbol=" + g_sym + ",ea_version=v1.2\n");
        FileWriteString(g_log,
    ...
    Print("v1.2 exit rule: basket closes at wavg +/- ", BasketTPPips, " pips. "
          "Single basket at a time. No auto-close on opposite signal.");
